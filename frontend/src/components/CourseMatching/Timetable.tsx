...
import {  resetTimetable } from "@/lib/courseActions";
...
import { updateUserTimetableColors } from "@/lib/courseUtils";
...
const Timetable: React.FC<TimetableProps> = ({ NUSModsURLs, currentUserName }: TimetableProps) => {
    ...
    useEffect(() => {
        const fetchColorAssignments = async () => {
            try {
                await updateUserTimetableColors(NUSModsURLs.filter(url => url.name === currentUserName)[0].url)
                const timetableColorAssignments = JSON.parse(localStorage.getItem('timetableColorAssignments') || "");
    ...
    return (
        <div>
            <LinkAdder changeColorAssignmentState={setAssignments}/>
            <div className="mb-10 mt-[30px] sm:mx-[20px] lg:mx-[80px]">
                ...
                <form action={async () => {
                    resetTimetable();
                    localStorage.setItem('timetableColorAssignments', "");
                    await updateUserTimetableColors(NUSModsURLs.filter(url => url.name === currentUserName)[0].url);
                    router.refresh();
                }} className="mt-4">
                ...
                {daysShortform.map((day, dayIndex) => {
                    return (
                        <div key={dayIndex.toString()} className="mb-2">
                            ...
                            <div className="grid bg-gray-100 rounded-t-md" style={{ gridTemplateColumns: 'repeat(15, minmax(0, 1fr))' }}>
