 ...
 */
async function createActivity(details: ActivityDetails, organiserId: string) : Promise<Activity> {
 ...
 */
async function searchActivities(search: string, pageNum: number, category: string | undefined, date: string | undefined, location: string | undefined) : Promise<Activity[]> {
    const where: any = {
        AND: []
    }
    if (category) {
...
async function searchActivity(id: string) : Promise<Activity | null> {
...
async function checkActivityEnrollment(activityId: string, userId: string): Promise<boolean> {
...
*/
async function addParticipant(activityId: string, userId: string) : Promise<Activity | void> {
 ...
 */
async function removeParticipant(activityId: string, userId: string) : Promise<Activity> {
    const update = await prisma.activity.update({
        where: {
            id: activityId
        },
 ...
 */
async function deleteActivity(activityId: string) : Promise<Activity> {
 ...
 */
async function checkIfOwner(activityId: string, userId: string): Promise<boolean> {
    const find = await prisma.user.findUnique({
        where: {
            id: userId
        },
...
async function getActivityParticipants(activityId: string) : Promise<string[]> {
    ...
    return activity?.participants.map((participant: User) => participant.name) ?? [];
}
 ...
 * @returns true if the activity is not full, false if it is full 
 */
async function countParticipants(activityId: string) : Promise<boolean> {
...
export { createActivity, getActivity, searchActivities, searchActivity, checkActivityEnrollment, addParticipant, removeParticipant,
    deleteActivity, checkIfOwner, countParticipants, getActivityParticipants
}